...
//class Line extends Point, Shape{}	// 자바는 다중 상속을 허용하지 않으므로 에러가 발생

// 인터페이스란 정적(static) 멤버 변수와 추상 메소드로만 구성된 클래스의 특별한 형태
interface Draw{
...
//class Line extends Draw {} 			//클래스는 인터페이스를 상속받을 수 없으므로 에러가 발생

//interface Graphics extends Point {}	//인터페이스는 클래스를 상속받을 수 없으므로 에러가 발생
// 클래스는 클래스끼리 인터페이스는 인터페이스끼리 상속을 시켜야 한다

interface Graphices extends Draw, Graphic {	//인터페이스는 다중 상속을 허용한다
	//아무런 내용도 가지지 않는 인터페이스를 표시(marker) 인터페이스라 부른다
}

// 클래스 선언 시 인터페이스에서 정의한 내용을 사용하려면 상속(exnteds)을 사용할 수 없고 구현(implements)을 사용한다
